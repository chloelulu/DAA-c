...
temp <- load('/research/bsi/projects/staff_analysis/m216453/correlated/code/VaginalIntroitus_V35.RData',envir=.GlobalEnv)
temp0 <- load('/research/bsi/projects/staff_analysis/m216453/correlated/code/VaginalIntroitus_V35_dirmult.RData',envir=.GlobalEnv)
...
paras$resdir <- resdir
paras$prefix <- prefix
#paras$methods <- c('lme','glmernb','GLMMPQL','ZIBR','LDM','NBMM','ZIGMM','ZINBMM','glmmadaptive','glmmTMB','LinDA')
...
setwd(resdir)
res <- clsapply(900:1000, func, paras, queque='1-day', tempdir=file.path(resdir, 'tmpC'))
setwd(resdir)
