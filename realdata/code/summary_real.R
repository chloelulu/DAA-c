...
res00[is.na(res00)] <- 1
colnames(res00) <- gsub('_smoker','',colnames(res00))
sig.05 <- sum(apply(res00, 1, function(x) sum(x<=0.05)) >0)/nrow(res00)
sig.1 <- sum(apply(res00, 1, function(x) sum(x<=0.1)) >0)/nrow(res00)
sig.2 <- sum(apply(res00, 1, function(x) sum(x<=0.2)) >0)/nrow(res00)
names(sig.05) <- paste0('smoker_0.05')
names(sig.1) <- paste0('smoker_0.1')
names(sig.2) <- paste0('smoker_0.2')
sigs <- c(sig.05, sig.1, sig.2)
...
sig.1 <- sum(apply(res00, 1, function(x) sum(x<=0.1)) >0)/nrow(res00)
sig.2 <- sum(apply(res00, 1, function(x) sum(x<=0.2)) >0)/nrow(res00)
names(sig.05) <- paste0('Nicholas2013_0.05')
names(sig.1) <- paste0('Nicholas2013_0.1')
names(sig.2) <- paste0('Nicholas2013_0.2')
sigs <- c(sigs, sig.05, sig.1, sig.2)
...
sig.1 <- sum(apply(res00, 1, function(x) sum(x<=0.1)) >0)/nrow(res00)
sig.2 <- sum(apply(res00, 1, function(x) sum(x<=0.2)) >0)/nrow(res00)
names(sig.05) <- paste0('IBD2017: X_0.05')
names(sig.1) <- paste0('IBD2017: X_0.1')
names(sig.2) <- paste0('IBD2017: X_0.2')
sigs <- c(sigs, sig.05, sig.1, sig.2)
...
sig.1 <- sum(apply(res00, 1, function(x) sum(x<=0.1)) >0)/nrow(res00)
sig.2 <- sum(apply(res00, 1, function(x) sum(x<=0.2)) >0)/nrow(res00)
names(sig.05) <- paste0('IBD2017: interaction_0.05')
names(sig.1) <- paste0('IBD2017: interaction_0.1')
names(sig.2) <- paste0('IBD2017: interaction_0.2')
sigs <- c(sigs, sig.05, sig.1, sig.2)

sigs.df <- melt(sigs) %>% rownames_to_column('datasets') %>% 
  tidyr::separate(col = datasets, into = c('datasets','FDR'), sep = '_') 
sigs.df$datasets <- gsub('smoker','Smoker',sigs.df$datasets)
sigs.df$datasets <- gsub('Smoker','Smoker2010(X)',sigs.df$datasets)
sigs.df$datasets <- gsub('Nicholas2013','Nicholas2013(time)',sigs.df$datasets)
sigs.df$datasets <- gsub('IBD2017: X','IBD2017(X)',sigs.df$datasets)
sigs.df$datasets <- gsub('IBD2017: interaction','IBD2017(interaction)',sigs.df$datasets)
sigs.df <- within(sigs.df, datasets <- factor(datasets, levels = c('Smoker2010(X)','Nicholas2013(time)','IBD2017(X)','IBD2017(interaction)')))
sigs.df$FDR <- gsub('0.05','5%',sigs.df$FDR)
sigs.df$FDR <- gsub('0.1','10%',sigs.df$FDR)
sigs.df$FDR <- gsub('0.2','20%',sigs.df$FDR)
sigs.df <- within(sigs.df, FDR <- factor(FDR, levels = c('5%','10%','20%')))

sigs.df[sigs.df$datasets %in% c('Smoker2010(X)','Nicholas2013(time)'),][,3] %>%sort()
sigs.df[!(sigs.df$datasets %in% c('Smoker2010(X)','Nicholas2013(time)')),][,3] %>%sort()

ggplot(sigs.df, aes(x = datasets, y = (value)*100, fill = FDR)) + 
  geom_bar(position="dodge", stat="identity") + 
  facet_grid(.~ FDR) +
  theme_bw() + 
  # guides(fill=  T) + 
  scale_fill_brewer(palette = 'Set1') + 
  labs(x = '', y = 'Coverage(%)', fill = '') + 
  theme(axis.text.x = element_text(color = 'black', size = 14, angle = 90, hjust = 1),
        axis.text = element_text(color = 'black', size = 14),
        strip.text = element_text(color = 'black', size = 14),
        axis.title = element_text(color = 'black', size = 14),
        ...
        legend.text = element_text(color = 'black', size = 14),
        legend.position = 'none')
ggsave("/research/bsi/projects/staff_analysis/m216453/correlated/plot/coverage.pdf", width = 6, height = 5)




## Stability
setwd('/home/mayo/m216453/lu/correlated/Data/')
files.subset <- list.files('realsubset/')
files.original <- list.files('real/')

cor.rhos <- c()
for(pttn in c('IBD','Nicholas','smoker')){
  for(file in files.subset[grep(pttn,files.subset)]){
    cat(file,'\n')
    wrapper.obj <- NULL
    load(paste0('realsubset/',file))
    subset.res <- wrapper.obj$res
    if(length(grep('LinDA',file))>0){
      colnames(subset.res) <- paste0(gsub('.res.Rdata','',file),'-',colnames(subset.res),'.padj')
    }else{
      colnames(subset.res) <- paste0(gsub('.res.Rdata','',file),'-',colnames(subset.res))
    }
    
    wrapper.obj <- NULL
    load(paste0('real/',paste0(strsplit(file,split = '_')[[1]][c(1,3)], collapse = '_')))
    original.res <- wrapper.obj$res
    colnames(original.res) <- paste0(gsub('.res.Rdata','',file),'-original-',colnames(original.res))
    join.res <- merge(subset.res, original.res, by = 0) %>% column_to_rownames('Row.names')
    if(pttn =='IBD'){
      for(tts in c('X.padj','interaction.padj')){
        tmp.df <- join.res[,colnames(join.res)[grep(tts,colnames(join.res))],drop =F]
        cor.rho <- cor.test(tmp.df[,1],tmp.df[,2], method = 'spearman')$estimate
        names(cor.rho) <- paste0(gsub('.res.Rdata','',file),'-',tts)
        cor.rhos <- c(cor.rhos, cor.rho)
      }
    }else{
      cor.rho <- cor.test(join.res[,1],join.res[,2], method = 'spearman')$estimate
      names(cor.rho) <- gsub('.res.Rdata','',file)
      cor.rhos <- c(cor.rhos, cor.rho)
    }
  }
}
...
cor.df <- as.data.frame(cor.rhos) %>% rownames_to_column('datasets') %>% separate(datasets, into = c('method','iter','datasets'), sep = '_')
ords <- (aggregate( cor.rhos ~ method ,cor.df, function(x) median(x)) %>% arrange(cor.rhos))[,'method']
cor.df <- within(cor.df, method <- factor(method, levels = ords))
load('../Data/cols.Rdata')
ggplot(cor.df, aes(x = method, y = cor.rhos, fill = method)) + 
  geom_boxplot(outlier.size = 0.5) + #facet_grid(.~ datasets) + 
  labs(x = '', y = 'Stability (Spearman rho)', fill = '') + 
  scale_fill_manual(values = cols) + 
  theme_bw() + 
  theme(axis.text.x = element_text(color = 'black', size = 14, angle = 90, hjust = 1),
        axis.text = element_text(color = 'black', size = 14),
        ...
        legend.text = element_text(color = 'black', size = 14),
        legend.position = 'none')
ggsave("/research/bsi/projects/staff_analysis/m216453/correlated/plot/stability.pdf", width = 6, height = 5)
data_summary(data = cor.df, formula = 'cor.rhos ~ method') %>% arrange( cor.rhos)
